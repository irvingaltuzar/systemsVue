...
<el-table v-loading="listLoading" :data="RequisitionsMyPersonal_Pendientes.filter(data => !search || data.id.toString().includes(search.toString()) ||  (data.user && data.user.toLowerCase().indexOf(this.search.toLowerCase()) > -1) ||
...
export default {
  ...
  computed: {
    // mix the getters into computed with object spread operator
     ...mapGetters([
      'RequisitionsMyPersonal_Pendientes',
    ])
  },
  created() {
   this.$store.dispatch("getRequisitionsMyPersonalPendientes");

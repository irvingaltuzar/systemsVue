<template>
  ...
  <el-dialog width="85%" center top="5vh" :visible.sync="dialogVisible" :title="dialogType==='edit'?'Validación Requisición':'Crear Archivo'">
      ...
      <el-card class="box-card" shadow="always">
        ...
        <el-container class="ml-5">
          ...
          <el-form-item label="Departamento:" class="ml-5">
           <el-input v-model="formRequisition.department" ></el-input>
           </el-form-item>
...
export default {
 ...
 CancelarModal() {
  ...
  //      if(this.formRequisition.status=="validacion"){
  ...
  //      }else{
    ...
    this.$store.dispatch('updateRequisition',formData).then(res => {
      ...
      const formAut = new FormData()
        ...
        formAut.append('comentarios', 'Se actualizo esta requisición')
  ...
  AutoRequisition(){
 this.$confirm('Se autorizaran las firmas requisición de personal, estas seguro de continuar, no podrás realizar cambios posteriormente?', 'Confirmar Autorización', {
      ...
      // formData.append('personal_autorizations',  JSON.stringify(this.$store.getters.PersonalStaffHigher))
           ...
           this.$store.dispatch('AutorizeRequisition',formData).then(res => {
           this.$notify({
        ...
        // const formAut = new FormData()
        // formAut.append('name_view', this.$route.name)
        // formAut.append('comentarios', 'Se autorizó esta requisición supervalidador')
   ...
   async ValidateRequisition(){
    if(this.personalFirmas.length >0){
      ...
      formData.append('personal_autorizations',  JSON.stringify(this.personalFirmas))
           ...
           this.$store.dispatch('ValidateRequisition',formData).then(res => {
           this.$notify({
          ...
          offset: 50
           ...
           const formAut = new FormData()
        formAut.append('name_view', this.$route.name)
        formAut.append('comentarios', 'Se validó esta requisición')
        formAut.append('evento', 'ValidateRequisition()')
         formAut.append('id_afectado', this.formRequisition.id)
        ...
        this.$store.dispatch("getRequisitionValidation").then(res => {
          ...
          this.$notify.error({
          title: 'Error',
          message: "No se puede validar requisicion problema con autorizacion de firmas",
        });
    }
      },

<template>
  <el-dialog :key="renderKey" width="50vw" :visible.sync="dialogVisible" center top="5vh" :title="dialogType==='edit'?'Detalles Justificación':'Crear Justificación Empleado'">
    <el-form ref="ruleForm" :model="ruleForm" label-width="145px" :rules="rules" label-position="left" :disabled="isDisabled">
      ...
      <el-form-item label="Tipo Justificación" prop="typeJus">
        ...
        <el-select v-model="ruleForm.typeJus" filterable placeholder="Selecciona tipo de justificación:" clearable>
          <el-option
            v-for="item in TypeJustifications"
            :key="item.id"
...
export default {
  ...
  computed: {
    TypeJustifications() {
      return this.$store.getters.typeJustifications
    }
  },

<template>
  <el-dialog :key="renderKey" width="50vw" :visible.sync="dialogVisible" center top="5vh" :title="dialogType==='edit'?'Detalles Justificación':'Solicitud Justificación'">
...
export default {
  ...
  beforeMount() {
    this.$root.$refs.ModalJustificationPersonal = this
  },
  created() {
    try {
  ...
  resetForm() {
    ...
    async addJustification() {
      ...
      this.$store.dispatch('addJustificationUser', formData).then(res => {
        this.$message({
        ...
        this.$refs.upload.clearFiles()
         this.$store.dispatch('getMyJustifications');

         const formAut = new FormData()
        formAut.append('name_view', this.$route.name)
        formAut.append('comentarios', 'Se agrego nueva justificación -> description:' + this.ruleForm.Comentarios)

...
export default {
  ...
  methods: {
    async getList(_page = 1){
      ...
      await Api.get('/rh/vacation/general-report',{
        params:this.headers
      })
      .then(response =>{
      ...
      .catch(error =>{
        console.log("Error => /rh/vacation/general-report => ",error);
        this.loading_get_list = false
    ...
    generalreport_excel(){
      ...
      this.general_data.data.forEach((permit,index) =>{
        ...
        this.report_excel.data.push({
          ...
          "type_period" : permit.type_period,
      ...
      */
      ...
      this.report_excel.file_name = "reporte_vacaciones_"+(moment(new Date()).format('YYYYMMDD_hhmmss'));
      ...
      formAut.append('id_afectado',"")
      formAut.append('evento', 'Vacaciones -> generalreport_excel()')

...
export default {
  ...
  methods: {
    async getList(_page = 1){
      ...
      await Api.get('/rh/vacation/staff-vacation-list',{
        params:this.headers
      })
      .then(response =>{
      ...
      .catch(error =>{
        console.log("Error => /rh/vacation/staff-vacation-list => ",error);
        this.loading_get_list = false
    ...
    async viewRecord(_id){
        ...
        .then(response =>{
            ...
            const formAut = new FormData()
            formAut.append('name_view', this.$route.name)
            formAut.append('comentarios', "Vacaciones del Personal")

  ...
  <div class="page-container">
    <el-card class="box-card" shadow="always">
      <div slot="header" class="clearfix text-center">
        <h3 class="text-center">Proveedores en proceso</h3>
      </div>
      <br>
      ...
      <br>
        ...
        <el-table-column prop="id" label="Folio" width="70" header-align="center" align="center">
          <template slot-scope="scope">
            ...
            <el-popover placement="top" width="350"  trigger="hover">
              ...
              {{ scope.row.efo }}
				...
				<el-table-column prop="date" label="Fecha de registro" width="80" header-align="center" align="center">
					<template slot-scope="scope">
						<span style="text-align:center">
            ...
            <template slot-scope="scope" v-if="(scope.row.update_user == null || scope.row.update_user == 0 )">
              <template v-if="scope.row.responsable.usuario_ad != user">
				...
				<el-table-column prop="is_open" width="150" label="Acciones" header-align="center" align="center">
					<template slot-scope="scope">
            <div class="align" style="display: inline-flex !important">
              <template v-if="scope.row.responsable.usuario_ad != user">
                <el-tooltip class="item align-tooltip" effect="dark" content="Agregar especialidad" placement="bottom">
                  <add-specialties-component :supplier_id="scope.row.id" :current_specialities="scope.row.specialities" :business_name="scope.row.business_name"/>
                </el-tooltip>
              </template>
              <template v-if="scope.row.responsable.usuario_ad != user">
                ...
                <template v-if="scope.row.responsable.usuario_ad != user && scope.row.update_user != null">
                <el-tooltip class="item align-tooltip" effect="dark" content="Devolver proveedor" placement="bottom" >
                  <el-button size="mini" type="warning" circle @click="getBackSupplier(scope.row.id)">
                   <md-icon class="size-xsmall">repeat</md-icon>
                  </el-button>
                </el-tooltip>
              </template>
              <template v-if="scope.row.responsable.usuario_ad != user">
				...
				<el-table-column prop="is_open" width="91" label="Historial" header-align="center" align="center">
					<template slot-scope="scope" v-if="scope.row.responsable.usuario_ad != user">
            <div class="align" style="display: inline-flex !important">
              <template>
                <el-tooltip class="item align-tooltip" effect="dark" content="Descargar archivos" placement="bottom">
                  <el-button size="mini" type="warning" circle @click="downloadZip(scope.row.files)">
            ...
            <el-popover placement="top" width="350"  trigger="hover">
              ...
              <el-button slot="reference" type="warning" style="font-size:11px;">
                Ver
              </el-button>
            </el-popover>
					</template>
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['Razón social / nombre', 'RFC', 'Tipo', 'Correo', 'Teléfono', 'Contacto Ventas', 'Estado', 'Unidad de negocio', 'EFO', 'Clasificación', 'Trabajos realizados', 'Estatus', 'Fecha de registro', 'Registró']
        const filterVal = ['business_name', 'rfc', 'type_supplier', 'email', 'phone', 'contact', 'state_name', 'responsable.company_name', 'efo', 'classification', '', 'status_name', 'date', 'responsable_name']
   ...
   async getBackSupplier(id){
    this.$confirm('Se devolvera el proveedor para su edición,estas segura de continuar?', 'Precaución', {
         ...
         Api.post('/suppliers/getBackSupplier',formData,config).then(res => {

          const formAut = new FormData()
          formAut.append('name_view', this.$route.name)
          formAut.append('comentarios', 'Regreso el proveedor al panel')
          formAut.append('id_afectado', id)
          formAut.append('evento', 'getBackSupplier()')

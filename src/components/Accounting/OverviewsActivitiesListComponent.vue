  ...
  <div class="page-container">
    <el-card class="box-card" shadow="always">
      <div slot="header" class="clearfix text-center">
        <h3>Opinión de cumplimiento</h3>
      </div>
      <div slot="header" class="clearfix text-end">
        <create-overview-activity-component />
      </div>
      ...
      <el-table class="m-1" style="width: 100%" :data="general_data.data" v-loading="loading_get_list">
        ...
        <el-table-column prop="accountant_name" label="Contador" width="180" header-align="center" align="center">
          ...
          <el-table-column prop="date" label="Fecha" width="180" header-align="center" align="center">
            <template slot-scope="scope">
              <span style="text-align:center;word-break:break-word">{{ scope.row.date | moment('ddd Do MMM YYYY') }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="overview_name" label="Opinión" width="180" header-align="center" align="center">
            <template slot-scope="scope">
              <span style="text-align:center;word-break:break-word">{{ scope.row.overview_name }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="comments" label="Comentario" width="180" header-align="center" align="center">
            <template slot-scope="scope">
              <span style="text-align:center;word-break:break-word;color:red">{{ scope.row.comments | moment('ddd Do MMM YYYY') }}</span>
            </template>
          </el-table-column>
      </el-table>
      <br>
...
export default {
  name: 'OverviewsActivitiesListComponent',
  data: () => ({
    general_data:{},
    headers:{
      limit:20,
  ...
  methods: {
    ...
    async getList(_page = 1){
      ...
      await Api.get('/accounting/fetch-overviews-accounting',{
        params:this.headers
      })
      .then(response =>{
      ...
      .catch(error =>{
        console.log(error," => ",'/accounting/fetch-overviews-accounting');
        this.loading_get_list= false
      });

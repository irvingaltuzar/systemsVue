  ...
  <div class="page-container">
    <el-card class="box-card" shadow="always">
      ...
      <div slot="header" class="clearfix text-end">
        <create-interim-payment-component />
      </div>
      ...
      <el-table class="m-1" style="width: 100%" :data="general_data.data" v-loading="loading_get_list">
        ...
        <el-table-column prop="diot_date" label="Fecha DIOT" width="180" header-align="center" align="center">
          <template slot-scope="scope">
            <span style="text-align:center;word-break:break-word">{{ scope.row.diot_date | moment('ddd Do MMM YYYY') }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="diot_id_transaction_receipt" label="Folio DIOT" width="180" header-align="center" align="center">
          <template slot-scope="scope">
            <span style="text-align:center;word-break:break-word">{{ scope.row.diot_id_transaction_receipt }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="dyp_date" label="Fecha DYP" width="180" header-align="center" align="center">
          <template slot-scope="scope">
            <span style="text-align:center;word-break:break-word">{{ scope.row.dyp_date | moment('ddd Do MMM YYYY') }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="dyp_id_transaction_receipt" label="Folio DYP" width="180" header-align="center" align="center">
          <template slot-scope="scope">
            <span style="text-align:center;word-break:break-word">{{ scope.row.dyp_id_transaction_receipt }}</span>
          </template>
        </el-table-column>
      </el-table>
      <br>
...
export default {
  name: 'InterimPaymentsListComponent',
  data: () => ({
    general_data:{},
    headers:{
      limit:20,
  ...
  methods: {
    ...
    async getList(_page = 1){
      ...
      // this.headers.month = moment(this.headers.month).subtract(1, 'month').format('MM');
      this.loading_get_list= true;
      await Api.get('/accounting/fetch-interim',{
        params:this.headers
      })
      .then(response =>{
      ...
      .catch(error =>{
        console.log(error," => ",'/accounting/fetch-interim');
        this.loading_get_list= false
      });
    },
